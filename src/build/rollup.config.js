...
// Plugins for single-file IIFE builds (no multi-entry merging needed).
function makeSinglePlugins() {
	return [
		babel( {
			presets: babelConfig.presets,
			browserslistEnv: 'bs5',
...
const conditionalScripts = [ 'translation-search', 'vtt-player' ].map( ( name ) => ( {
	...
	output: {
		banner: banner( '' ),
		file: path.resolve( __dirname, `../../js/${ name }.js` ),
		format: 'iife',
	},
	plugins: makeSinglePlugins(),
} ) );
