(function () {
  ...
  function playCue(startInp, endInp, row) {
  ...
  function parseVTT(text) {
    ...
    while (i < lines.length) {
      ...
      let id = '';
      if (!lines[i].includes('-->')) { id = lines[i].trim(); i++; }
      if (i >= lines.length || !lines[i].includes('-->')) { i++; continue; }
      const timingLine = lines[i]; i++;
  ...
  function autoSave() {
    ...
    fetch(`${REST_URL}${TRANSLATION_ID}`, {
      ...
      body: JSON.stringify({ vtt: buildVTT() }),
    })
      .then(r => r.json())
      .then(data => {
        if (data.success) {
  ...
  function renderTranscript() {
    ...
    cues.forEach((cue) => {
      ...
      textSpan.addEventListener('click', () => {
        playCue(row.querySelector('.vtt-start-inp'), row.querySelector('.vtt-end-inp'), row);
      });
  ...
  function scrollRowIntoView(row) {
    ...
    if (rowTop > playerBottom + 4) {
      window.scrollBy({ top: rowTop - playerBottom - 8, behavior: 'smooth' });
    }
  }
  ...
  audio.addEventListener('timeupdate', () => {
    ...
    for (const row of transcript.querySelectorAll('.vtt-phrase-row')) {
      const start = parseSec(row.querySelector('.vtt-start-inp').value);
      const end   = parseSec(row.querySelector('.vtt-end-inp').value);
      if (t >= start && t < end) { found = row; break; }
    }
  ...
  function showStatus(msg) {
    ...
    statusTimer = setTimeout(() => { statusMsg.textContent = ''; }, 3500);
  }
